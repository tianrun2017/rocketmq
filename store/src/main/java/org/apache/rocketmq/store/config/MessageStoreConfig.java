...
public class MessageStoreConfig {
    ...
    // CommitLog file size,default is 1G ,commitlog 文件的大小，默认为1G。
    private int mappedFileSizeCommitLog = 1024 * 1024 * 1024;
    // ConsumeQueue file size,default is 30W
    // consumeQueueSize，ConsumeQueue 存放的是定长的信息（20个字节，偏移量、size、tagscode）,
    // 默认30w * ConsumeQueue.CQ_STORE_UNIT_SIZE。
    ...
    // flush data to disk
    // 刷写 CommitLog 的间隔时间，
    // RocketMQ 后台会启动一个线程，将消息刷写到磁盘，这个也就是该线程每次运行后等待的时间，默认为500毫秒。
    ...
    // How many pages are to be flushed when flush CommitLog 每次 flush commitlog 时最小发生变化的页数
    private int flushCommitLogLeastPages = 4;
    // How many pages are to be committed when commit data to file 每一次 commitlog 提交任务至少需要的页数。
    private int commitCommitLogLeastPages = 4;
    // Flush page size when the disk in warming state 用字节0填充整个文件，每多少页刷盘一次，默认4096，异步刷盘模式生效
    private int flushLeastPagesWhenWarmMapedFile = 1024 / 4 * 16;
    // How many pages are to be flushed when flush ConsumeQueue 一次刷盘至少需要的脏页数量，默认为2，针对 consuequeue 文件
